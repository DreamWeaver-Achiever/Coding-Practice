...
long long int houseRobber(vector<int>& houses)
{
    ...
    int totalHouses = houses.size();
        ...
        //     }
        // }
        vector<int> firstHouseSeries(houses.begin(), houses.end() - 1);
        int robFirstHouse = maximumRobbery(firstHouseSeries); 
        vector<int> lastHouseSeries(houses.begin() + 1, houses.end());
         int robLastHouse = maximumRobbery(lastHouseSeries);
